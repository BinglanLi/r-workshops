...
The teaching material of this workshop is adapted from https://seandavi.github.io/ITR/index.html.
...
x != 5
x == 5
# store the logical operation output in a variable
x_gt_5 <- (x > 5)
x_gt_5
class(x_gt_5)
```
...
Lists contain objects that can be of different types, even including other lists.
...
Our `fhs_data` variable is a data.frame. The data set is fairly large, so we will not be
...
fhs_high_bp <- fhs_data[fhs_data$SBP >= 130 & fhs_data$DBP >= 130, ]
```
...
```{r}
# save to file
write.csv(fhs_high_bp, "framingham_high_blood_pressure.txt", sep = ",", quote = FALSE, col.names = TRUE, row.names = F)

# save one or more object to RData
save(fhs_high_bp, file = "framingham_high_blood_pressure.RData")
...
R comes with various libraries. Libraries comes with functions that is written and most of the time, painstakingly tested functions. Libraries make your data analysis and coding tasks easier. 
...
Both `dplyr` and `ggplot2` are parts of a larger R library called `tidyverse`, which provides a collection of R function for data science. 
...
calc_sum_squares <- function(a_vector){
    # this function calculates the the sum of the square of variation
...
R comes with some default plotting function, which is sufficient for preliminary data analysis. For publication-ready figures, `ggplot2()` and other libraries are recommended.
